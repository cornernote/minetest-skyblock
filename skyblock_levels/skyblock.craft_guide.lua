...
skyblock.craft_guide.image_button_link = function(stack_string)
	...
	return tostring(stack_string)..';craft_guide_'..tostring(new_node_name)..';'..group
end
...
local get_formspec = function(player_name, node_name, fields)
	...
	node_name = node_name:gsub('craft_guide_', '')

	-- the player may ask for recipies of ingredients to the current receipe
	if fields then
	...
	if (fields and fields.craft_guide_prev_receipe and receipe_nr > 1) then
		receipe_nr = receipe_nr - 1
	elseif (fields and fields.craft_guide_next_receipe and receipe_nr < #res) then
		receipe_nr = receipe_nr + 1
	end
	...
	if( res and receipe_nr > 1 ) then
		formspec = formspec..'button[3.8,5.5;1,0.5;craft_guide_prev_receipe;prev]'
	end
	if( res and receipe_nr < #res ) then
		formspec = formspec..'button[4.0,5.5;1,0.5;craft_guide_next_receipe;next]'
	end
...
skyblock.craft_guide.on_receive_fields = function(player, formname, fields)
	...
	for k,v in pairs(fields) do
		if string.match(k, 'craft_guide_') then
			player:set_inventory_formspec(get_formspec(player:get_player_name(), k, fields))
			return
		end
	end
end
